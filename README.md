...
| --- | --- |
| `15` | 早期原型，入口不完整 |
| `16` / `16.1` | 模块化中间版，块大小仍写死 |
| `16.2` | 本文方法算法定型（`s1,s2` 参数化，seed=42） |
| **`16.2批量测试`** | **论文实验用最终代码**：BOSSBase / BOWS-2 / UCID 批量结果、多进程、辅助信息溢出处理、种子稳健性实验 |

本仓库由 `16.2批量测试` 整理而来，并做了 GitHub 发布所需的清理：
...
- `16.3*`：单向移位 / 固定块大小等消融
- `16.4(PSA方法)`、`16.5(MPSA方法)`、`16.6(rotation)`、`16.7(min_max)`：对比方法
